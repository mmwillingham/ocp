...
# Enable Podman Restart Service for host reboot persistence
...
# ------------------------------------------------------------------------------
# 1. OPTIONAL: Rename CatalogSource in Generated Manifests
# Note: oc-mirror v2 generates default CatalogSource names like "cs-redhat-operator-index-v4-20".
# You can optionally shorten or rename this before applying to the cluster.
# ------------------------------------------------------------------------------
OLD_CATALOG_NAME="cs-redhat-operator-index-v4-20"
NEW_CATALOG_NAME="my-operators"

if [ -d "${RESOURCE_DIR}" ]; then
  echo "[INFO] Renaming CatalogSource '${OLD_CATALOG_NAME}' -> '${NEW_CATALOG_NAME}' in local manifests..."
  sed -i "s/${OLD_CATALOG_NAME}/${NEW_CATALOG_NAME}/g" "${RESOURCE_DIR}"/*.yaml 2>/dev/null || true
fi

# 2. Apply CatalogSource (OLM v0) and ClusterCatalog (OLM v1) manifests
find "${RESOURCE_DIR}" -type f \( -name "cs-*.yaml" -o -name "cc-*.yaml" -o -name "*catalog*.yaml" \) -exec oc apply -f {} \;

# 3. Disable Default Public OperatorHub Catalog Sources
cat << EOF | oc apply -f -
apiVersion: config.openshift.io/v1
kind: OperatorHub
metadata:
...
# ------------------------------------------------------------------------------
# 4. Wait for Custom CatalogSources Readiness
# ------------------------------------------------------------------------------
echo "--------------------------------------------------"
echo "[INFO] MONITORING CUSTOM CATALOGSOURCE READINESS..."
echo "--------------------------------------------------"

# Dynamically discover custom applied CatalogSource names (e.g. cs-* or my-operators)
...
if [ -z "${CUSTOM_CATALOGS}" ]; then
  echo "[WARN] No custom CatalogSources found in openshift-marketplace."
else
...
# NOTE (OPTIONAL): Set CATALOG_SOURCE to match your CatalogSource metadata.name from Section 7.
# If you ran the optional rename step in Section 7, use "my-operators".
# If you kept the default name generated by oc-mirror v2, use "cs-redhat-operator-index-v4-20".
CATALOG_SOURCE="my-operators"

echo "--------------------------------------------------"
echo "[INFO] DEPLOYING OPENSHIFT UPDATE SERVICE (OSUS)..."
echo "[INFO] Using CatalogSource: ${CATALOG_SOURCE}"
echo "--------------------------------------------------"
...
## Section 11: Remove Default Templates and Helm charts (Optional)
```
echo "--------------------------------------------------"
echo "[INFO] CLEANING UP SOFTWARE CATALOG CONTENT..."
echo "--------------------------------------------------"

# 1. Disable Default Red Hat Helm Chart Repository
cat << 'EOF' | oc apply -f -
...
# 2. Disable Cluster Samples Operator (Removes Default OpenShift Templates)
cat << 'EOF' | oc apply -f -
...
# 3. Restrict Web Console Developer Catalog (Hides Non-Operator Items in UI)
cat << 'EOF' | oc apply -f -
apiVersion: operator.openshift.io/v1
kind: Console
metadata:
...
# ------------------------------------------------------------------------------
echo "--------------------------------------------------"
echo "[INFO] VERIFYING CATALOG RESTRICTION CONFIGURATION..."
echo "--------------------------------------------------"

# Check A: Verify Helm Repository Status
HELM_STATUS=$(oc get helmchartrepository redhat-helm-charts -o jsonpath='{.spec.disabled}' 2>/dev/null)
if [ "${HELM_STATUS}" == "true" ]; then
  echo "[PASS] Red Hat Helm chart repository is disabled."
else
  echo "[FAIL] Failed to disable Red Hat Helm chart repository."
  exit 1
fi

# Check B: Verify Samples Operator State
SAMPLES_STATE=$(oc get configs.samples.operator.openshift.io cluster -o jsonpath='{.spec.managementState}' 2>/dev/null)
if [ "${SAMPLES_STATE}" == "Removed" ]; then
  echo "[PASS] Cluster Samples Operator is set to 'Removed'."
else
  echo "[FAIL] Cluster Samples Operator management state is not 'Removed'."
  exit 1
fi

# Check C: Verify Console Customization
CONSOLE_DISABLED=$(oc get console.operator.openshift.io cluster -o jsonpath='{.spec.customization.developerCatalog.types.disabled}' 2>/dev/null)
if echo "${CONSOLE_DISABLED}" | grep -q "HelmChart"; then
  echo "[PASS] Web Console developer catalog UI filters applied successfully."
else
  echo "[FAIL] Web Console catalog customization pending or missing."
  exit 1
fi
